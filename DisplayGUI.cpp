...
static const char* EXPECTED_PY_SHA256 = "99de912e081067d88815cf13d280bdf10db6a37509c8d88172f4bff6684f0169";
static const std::wstring kAllowedRoot = L"C:\\Users\\CMU\\RawRecords";
...
static bool VerifyFileSHA256(const AnsiString& filePath, const char* expectedHashHex) {
    ...
    if (CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT) &&
		...
		if (CryptGetHashParam(hHash, HP_HASHVAL, hash, &hashLen, 0)) {
            ...
            if (_stricmp(hashHex, expectedHashHex) == 0)
                result = true;
        }
...
static uint32_t CalcCRC32(const AnsiString& str) {
    return crc32(0, reinterpret_cast<const Bytef*>(str.c_str()), str.Length());
}

bool IsPathAllowed(const AnsiString& filePath) {
    try {
        std::filesystem::path absPath = std::filesystem::weakly_canonical(std::filesystem::absolute(filePath.c_str()));
        std::filesystem::path rootPath = std::filesystem::weakly_canonical(kAllowedRoot);

        std::wstring absStr = absPath.wstring();
        std::wstring rootStr = rootPath.wstring();

        if (!rootStr.empty() && (rootStr.back() == L'.' || rootStr.back() == L'\\' || rootStr.back() == L'/')) {
            while (!rootStr.empty() && (rootStr.back() == L'.' || rootStr.back() == L'\\' || rootStr.back() == L'/'))
                rootStr.pop_back();
            rootStr += L'\\';
        }

        if (absStr.length() >= rootStr.length() && absStr.substr(0, rootStr.length()) == rootStr) {
            printf("[IsPathAllowed] Path allowed \n");
            return true;
        } else {
            printf("[IsPathAllowed] Disallowed the route \n");
            return false;
        }
    } catch (...) {
        return false;
...
void __fastcall TTCPClientRawHandleThread::HandleInput(void)
{
 ...
 // Form1->MsgLog->Lines->Add(StringMsgBuffer);
  ...
  {
    __int64 CurrentTime = GetCurrentTimeInMsec();
    uint32_t crc = CalcCRC32(StringMsgBuffer);
    AnsiString line = IntToStr(CurrentTime) + "," + StringMsgBuffer + "," + IntToStr((int)crc);
    Form1->RecordRawStream->WriteLine(line);
  }
...
{
    if (RawConnectButton->Caption == "Raw Connect")
    {
        TPasswordForm* PasswordForm = new TPasswordForm(this);
        try
        {
            PasswordForm->ShowModal();
            if (PasswordForm->Confirmed)
            {
                try
                {
                    std::unique_ptr<TIdTCPClient> tempClient(new TIdTCPClient(NULL));
                    std::unique_ptr<TIdSSLIOHandlerSocketOpenSSL> tempHandler(new TIdSSLIOHandlerSocketOpenSSL(NULL));
                    
                    tempHandler->SSLOptions->Method = sslvTLSv1_2;
                    tempHandler->SSLOptions->RootCertFile = "server1.crt";
                    tempHandler->SSLOptions->Mode = sslmClient;
                    tempHandler->SSLOptions->VerifyMode = TIdSSLVerifyModeSet() << sslvrfPeer << sslvrfFailIfNoPeerCert;
                    tempHandler->SSLOptions->VerifyDepth = 9;
                    tempHandler->PassThrough = False;
                    tempClient->IOHandler = tempHandler.get();
                    tempClient->Host = RawIpAddress->Text;
                    tempClient->Port = 30002;
                    tempClient->Connect();

                    tempClient->IOHandler->DefStringEncoding = IndyTextEncoding_UTF8();
                    ...
                    if (response == "OK")
                    {
                        ...
                        TCPClientRawHandleThread->IdTCPClient = tempClient.release();
                        TCPClientRawHandleThread->SSLHandler = tempHandler.release();
                        ...
                        {
                            delete TCPClientRawHandleThread;
...
void __fastcall TForm1::RawRecordButtonClick(TObject *Sender)
{
 ...
 {
  // Add path information message before showing save dialog
  AnsiString message = AnsiString("Files need to be saved to absolute path:\n") + AnsiString("C:\\Users\\CMU\\RawRecords");
  MessageDlg(message, mtInformation, TMsgDlgButtons() << mbOK, 0);

  if (RecordRawSaveDialog->Execute())
   {
    if (!IsPathAllowed(RecordRawSaveDialog->FileName)) {
        ShowMessage("This path is not allowed. It can only be saved under the Project folder.");
        return;
    }
...
__fastcall TTCPClientRawHandleThread::TTCPClientRawHandleThread(bool CreateSuspended)
...
void __fastcall TTCPClientRawHandleThread::Execute(void)
{
    ...
    if (!UseFileInsteadOfNetwork && (!IdTCPClient || !SSLHandler))
    {
        Synchronize(&StopTCPClient);
        return;
    }
    ...
    while (!Terminated)
    {
        try
        {
            if (UseFileInsteadOfNetwork)
            {
                try
                {
                    String line = Form1->PlayBackRawStream->ReadLine();
                    ...
                    try 
                    {
                        ...
                        if (tokens->Count >= 3) 
                        {
                            __int64 fileTime = StrToInt64(tokens->Strings[0]);
                            AnsiString msg = tokens->Strings[1];
                            uint32_t fileCRC = StrToInt(tokens->Strings[2]);
                            uint32_t calcCRC = CalcCRC32(msg);

                            if (fileCRC != calcCRC) 
                            {
                                ShowMessage("CRC mismatch! Raw file may be corrupted.");
                                if (!Terminated) Synchronize(&StopPlayback);
                                break;
                            }
                            ...
                            if (!Terminated) Synchronize(&HandleInput);
                        }
                        else 
                        {
                            ShowMessage("Raw file format error.");
                            if (!Terminated) Synchronize(&StopPlayback);
                            break;
                        }
                    }
            ...
            else
            {
                if (!IdTCPClient->Connected())
                {
                    if (!Terminated) Synchronize(&StopTCPClient);
                    break;
                }

                try
                {
                    StringMsgBuffer = IdTCPClient->Socket->ReadLn();
                    if (!StringMsgBuffer.IsEmpty() && !Terminated)
                    {
...
//--------------------------------------------------------------------------
	 static void RunPythonScript(AnsiString scriptPath,AnsiString args)
	 {
        if (!VerifyFileSHA256(scriptPath, EXPECTED_PY_SHA256)) {
			ShowMessage("Python script integrity check failed!\n");
			return;
		}
		STARTUPINFOA si;
